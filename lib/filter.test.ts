...
describe("combined filters", () => {
  it("preserves the ONLINE source row while excluding it from the public dataset", () => {
    expect(sourceData.classes).toHaveLength(192);
    expect(sourceData.classes.filter((item) => item.roomRaw === "ONLINE")).toHaveLength(1);
    expect(data.classes).toHaveLength(191);
  ...
  it.each([["A Session", 100], ["B Session", 49], ["C Session", 42]])("keeps the %s on-campus filter count correct", (session, count) => {
    expect(filterClasses(data.classes, { ...EMPTY_FILTERS, termId: "fall-2026", session })).toHaveLength(count);
  });
  it("accounts for all 191 on-campus classes across the three public session groups", () => {
  ...
  it("keeps the public Monday and 8:00 AM totals on-campus only", () => {
    expect(filterClasses(data.classes, { ...EMPTY_FILTERS, termId: "fall-2026", day: "Monday" })).toHaveLength(51);
